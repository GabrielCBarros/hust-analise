...
export const CONFIG_IA_INFORMACAO_SOBRE_AS_MENSAGENS = `CONFIGURAÇÃO SOBRE AS MENSAGENS: Eu quero que você analise conversas entre a minha empresa e meus clientes. A conversa será em formato JSON e está organizada da seguinte forma:
    1. Contem um array de mensagens com nome 'mensagens';
    2. Cada mensagem desse array está organizada da seguinte forma:'<IDENTIFICADOR> (<DATA_DA_MENSAGEM>): <TEXTO_DA_MENSAGEM>'
      2.1. '<IDENTIFICADOR>': Identificador de quem mandou a mensagem. Esse identificador pode ser de três tipos:
        2.1.1. 'Cliente': Se uma mensagem for mandada por um cliente para a empresa, a mensagem terá esse identificador no início da mensagem;
        2.1.2. 'Empresa por BOT': Se uma mensagem for mandada por um bot da empresa para um cliente, a mensagem terá esse identificador no início da mensagem;
        2.1.3. 'Empresa por <NOME_DO_ATENDENTE>': Se uma mensagem for mandada por um atendente da empresa para um cliente, a mensagem terá esse identificador no início da mensagem.
      2.2 '<DATA_DA_MENSAGEM>': Data que a mensagem foi enviada, no formato 'dd-MM-yyyy HH:mm:ss';
      2.3 '<TEXTO_DA_MENSAGEM>': Texto da mensagem;
...
export const CONFIG_IA_INFORMACAO_SOBRE_COMO_ANALISAR = `CONFIGURAÇÃO SOBRE COMO ANALISAR: A análise de mensagem deve ser feita da seguinte forma:
...
export const CONFIG_IA_INFORMACAO_SOBRE_COMO_RESPONDER = `CONFIGURAÇÃO SOBRE COMO RESPONDER: A análise feita deve ser retornada/respondida para mim em formato JSON. No retorno do JSON deve conter:
    ...
    4. Resumindo, o JSON deve ser do seguinte formato: {"suggestion": ["sugestao1 do cliente"],"complaint": ["reclamacao1 do cliente", "reclamacao2 do cliente"],"praise": []}.
